...
(function (global) {
  ...
  // The chain's clock counts nanoseconds since GENESIS, not since the Unix epoch,
  // so an `expires_at_ns` from Memphis is not a wall-clock instant. Read as one it
  // lands in 1970, every stored session looks expired, and a reload signs the
  // person out. A value before this floor (2020-01-01) cannot be an epoch
  // instant; only the local 30-minute cap applies then. The contract enforces
  // the real expiry on its own clock either way — this bound only decides when
  // the page stops presenting the token.
  var EPOCH_FLOOR_MS = 1577836800000;
  ...
  function expiryMsFrom(expiresAtNs) {
    ...
    if (parentMs < EPOCH_FLOOR_MS) return cap;        // chain-relative, not wall-clock
