...
import forms_families_a, forms_families_b  # noqa: E402
...
def main():
    ...
    catalogue = FORMS + forms_families_a.FORMS + forms_families_b.FORMS + cycle_forms(procedures, leadsheets, seed('movement_schedules'))
    ...
    ext = [at_version(f, 1) for f in catalogue if f['number'] > PRODUCT_FORMS]
    ...
    print(f'wrote {len(catalogue)} forms ({nfields} fields, {sum(1 for f in catalogue if f["kind"] == "letter")} letters) to forms/, '
          f'{len(product)} to {os.path.relpath(OUT)} (unchanged), {len(ext)} to {os.path.relpath(OUT_EXT)} (unchanged) and {len(v2)} at version 2 to {os.path.relpath(OUT_V2)} ({frozen} newly frozen); '
