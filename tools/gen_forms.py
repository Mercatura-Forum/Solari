...
def write_module(path, forms, doc):
    ...
    if os.path.exists(path) and path == OUT:
        ...
        if old != text:
            raise SystemExit(f'{os.path.relpath(path)} would change: the fourteen are signed under this module and must be regenerated byte for byte')
        return
...
def main():
    ...
    catalogue = FORMS + forms_families_a.FORMS + forms_families_b.FORMS + cycle_forms(procedures, leadsheets)
    ...
    product = [f for f in catalogue if f['number'] <= PRODUCT_FORMS]
    ext = [f for f in catalogue if f['number'] > PRODUCT_FORMS]
    ...
    print(f'wrote {len(catalogue)} forms ({nfields} fields, {sum(1 for f in catalogue if f["kind"] == "letter")} letters) to forms/, '
          f'{len(product)} to {os.path.relpath(OUT)} (unchanged) and {len(ext)} to {os.path.relpath(OUT_EXT)}; '
